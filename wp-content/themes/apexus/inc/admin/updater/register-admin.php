<?php
/**
 * The Apexus_Register initiate the theme engine
 */

if( !defined( 'ABSPATH' ) ) 
	exit; // Exit if accessed directly

class Apexus_Register {
	...
	public function __construct( $config = array(), $strings = array() ) {
		...
		$config = wp_parse_args( $config, array(
			...
			'theme_slug'     => apexus()->get_slug(),
			'theme_name'     => apexus()->get_name(),
			'version'        => '',
	...
	function updater() {
		...
		if ( !class_exists( 'Apexus_Updater' ) ) {
			// Load our custom theme updater
			require get_template_directory() . '/inc/admin/updater/updater-class.php';
		}
	...
	function admin_error() {
		echo '<div class="error"><p>' . sprintf( wp_kses_post( esc_html__( 'The %s theme needs to be registered. %sRegister Now%s', 'apexus' ) ), apexus()->get_name(), '<a href="' . admin_url( 'admin.php?page=pxlart') . '">' , '</a>' ) . '</p></div>';
	}
	
	function admin_notice() {
		echo '<div class="notice"><p>'.esc_html__( 'Purchase code is invalid. Need a license for activation', 'apexus' ).'</p></div>';
	}
	...
	function messages($merlin = false){
		...
		if ( ! $purchase_code ){
			...
			<div class="pxl-dsb-box-head-inner">
				<h6><?php echo esc_html__( 'Register License', 'apexus' ) ?></h6>
			</div>
			<?php 
			$this->form();
			?>
			<div class="pxl-dsb-box-foot">
				<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e( 'Can’t find your purchase code?', 'apexus' ); ?></a>
			</div>
			<?php 
		}else{  
	...
	function check_license($merlin) {
		...
		if ( false === $license_data->success ) {
			switch ( $license_data->error ) {
				case 'missing':
					$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'apexus' );
				break;
				case 'item_name_mismatch':
					$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'apexus' ), $this->theme_name );
				break;
				case 'license_exists':
					$message = esc_html__( 'Your license is not active for this URL.', 'apexus' );
				break;
				default:
					$message = esc_html__( 'An error occurred, please try again.', 'apexus' );
				break;
			}
			?>
			<div class="pxl-dsb-confirmation fail">
				<h6><?php echo esc_html__( 'Active false', 'apexus' ) ?></h6>
				<p><?php echo wp_kses_post( $message ) ?> <a href="<?php echo esc_url($pxl_server_info['docs_url']) ?>" target="_blank"><?php echo esc_html__( 'our help center', 'apexus' ) ?></a> or <a href="<?php echo esc_url($pxl_server_info['support_url']) ?>" target="_blank"><?php echo esc_html__( 'submit a ticket', 'apexus' ) ?></a></p>
			</div>
			<?php $this->form(); ?>
			<div class="pxl-dsb-box-foot">
				<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e( 'Can’t find your purchase code?', 'apexus' ); ?></a>
			...
			if ( 'valid' === $license_data->license ) {
				...
				<div class="pxl-dsb-box-head"> 
					<div class="pxl-dsb-confirmation success">
						<h6><?php echo esc_html__( 'Thanks for the verification!', 'apexus' ) ?></h6>
						<p><?php echo esc_html__( 'You can now enjoy and build great websites', 'apexus' ) ?></p>
					</div> 

					<div class="pxl-dsb-deactive" style="display: none;">
						<form method="POST" action="<?php echo admin_url( 'admin.php?page=pxlart' )?>">
							<input type="hidden" name="action" value="removekey"/>
							<button class="btn button" type="submit"><?php esc_html_e( 'Remove Purchase Code', 'apexus' ) ?></button>
						</form>
					</div> 
				</div> 
				<?php 
	...
	function form() {
		...
		<form action="options.php" method="post" class="pxl-dsb-register-form">
			...
			<input id="<?php echo esc_attr($this->theme_slug)?>_purchase_code" name="<?php echo esc_attr($this->theme_slug)?>_purchase_code" type="text" value="<?php echo esc_attr( $license ); ?>" placeholder="<?php esc_attr_e( 'Enter your purchase code', 'apexus' ); ?>">
			<input type="submit" class="res-purchase-code" value="<?php esc_attr_e( 'Register your purchase code', 'apexus' ) ?>">
		</form>
		<?php
	}
