...
class PXL_Post_Metabox
{
    ...
    function save_meta_boxes($post_id, $post)
    {
        ...
        if (in_array($post_type, $this->post_types) && !empty($_POST[$this->panels[$post_type]['args']['opt_name']])) {
            ...
            //$validate = $redux->_validate_values($data_to_save, $data_to_compare, $sections); // hide from version 1.2.6
            $validate = $redux->validate_class->validate( $data_to_save, $data_to_compare, $sections ); // add from version 1.2.6
            ...
            foreach ($data_to_save as $key => $value) {
                ...
                //if (array_key_exists($key, $data_to_compare)) { // hide from version 1.2.6
