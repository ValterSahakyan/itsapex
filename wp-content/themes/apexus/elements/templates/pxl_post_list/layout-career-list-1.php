...
$query_result = pxl_get_posts_of_grid('career', ['source' => $source, 'orderby' => $orderby, 'order' => $order, 'limit' => $limit, 'post_ids' => $post_ids, 'post_not_in' => $post_ids_unselected ], $tax);

extract($query_result);
 
$post_type            = $widget->get_setting('post_type','career');
$layout               = $widget->get_setting('layout_'.$post_type, 'career-list-1');
...
$load_more = array(
    ...
    'img_size'            => $widget->get_setting('img_size','758x590'),
    ...
    'show_infor'       => $widget->get_setting('show_infor'),
...
$wrap_attrs = [
    'id'               => $html_id,
    'class'            => trim('pxl-grid pxl-career-list layout-'.$layout),
...
if( count($posts) <= 0){
    echo '<div class="pxl-no-post-list">'.esc_html__( 'No Post Found', 'apexus' ). '</div>';
    return;
}
?>
...
<div <?php pxl_print_html($widget->get_render_attribute_string( 'wrapper' )) ?>>
        ...
        <div class="post-list-toolbar row align-items-center justify-content-between">
            <div class="col col-sm-auto">
                <?php 
                    ...
                    printf(
                        '<span class="result-count">%1$s %2$s</span>',
                        $limitpage,
    ...
    <?php endif; ?>
    <div class="pxl-grid-inner d-flex-wrap relative"> 
