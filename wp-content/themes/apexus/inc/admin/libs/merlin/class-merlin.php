...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		if ( true !== $this->dev_mode ) {
			...
			if ( $already_setup ) {
				//return;
				//fix Sorry, you are not allowed to access this page. after import finish and reload
				if ( isset($_GET['page']) && sanitize_text_field($_GET['page']) === 'pxlart-setup' ) {
		            wp_safe_redirect( admin_url( 'admin.php?page=pxlart' ) );
		            exit;
		        }
	...
	public function add_admin_menu() {
		//fix Sorry, you are not allowed to access this page. after import finish and reload
		if ( get_option( 'merlin_' . $this->slug . '_completed' ) || !empty( get_option('pxl_import_demo_id',false) ) ){
	        return;
	    }
		// Strings passed in from the config file.
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'apexus' ),
		);
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'apexus' ),
		...
		if ( $this->license_step_enabled ) {
			$this->steps['license'] = array(
				'name' => esc_html__( 'License', 'apexus' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'apexus' ),
		...
		$this->steps['content'] = array(
			'name' => esc_html__( 'Content', 'apexus' ),
			'view' => array( $this, 'content' ),
		);

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'apexus' ),
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'apexus' ); ?>">
										<?php esc_html_e( 'Required', 'apexus' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( sanitize_title($_POST['slug']) === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'apexus' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( sanitize_title($_POST['slug']) === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'apexus' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( sanitize_title($_POST['slug']) === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'apexus' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			...
			$json['message'] = esc_html__( 'Installing', 'apexus' );
			wp_send_json( $json );
		} else {
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'apexus' ),
				)
			);
		}
